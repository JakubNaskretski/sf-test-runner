...
- **Tests** — the target org as a dropdown, ⟳ to refresh the org list and ＋ to log in to another org; a production org gets a warning bar above the tree. All/Selected tabs, a search box and a source filter sit above a tree of test classes with tri-state checkboxes down to the method. **Select tests for active class** ticks the tests for the class open in the editor, and a “with coverage” checkbox decides whether the run asks for `--code-coverage`. A “with logs” checkbox keeps each test method's debug log: before the run the plugin makes sure your user has an active USER_DEBUG trace flag (extending yours, creating one on an `SfTestRunner` debug level that logs Apex at DEBUG and nothing else, or pointing yours at that level when its own would swallow `System.debug`), and afterwards a **log** button next to each method opens its log in an editor tab — the `System.debug` lines first, the full log underneath, ready to read, copy or save. The run row is **Run Selected**, **All Local** (`--test-level RunLocalTests`, every test in the org except managed-package ones) and **All in Org** (managed packages included). While a run is on you see done/total, the elapsed time and a **Cancel** button.
...
- **Coverage** — after a coverage run the view leads with the classes the run was aimed at, not everything it touched. Targets are resolved per test class: **Declared** from `@IsTest(testFor='ApexClass:Foo')` (API v66+, and it names triggers too), else **By name** from the `FooTest`/`TestFoo`/`Foo_Test`/`FooTests` conventions when that class really was covered, else **By truncated name** for a class too long to have a conventional test class — matched on a prefix, declining on a tie. The headline and the bar measure those classes, with the 75% deploy floor dimmed to say it is an org-wide rule; the whole-run average sits in the legend. A declared target the run never reached shows as `not exercised`, and everything else folds into **Also covered**. A row opens the class or trigger; its cloud button loads that one class's stored coverage from the org's last run, whoever ran it, labelled as such. Covered and uncovered lines are painted in the editor with a gutter bar and overview-ruler marks, the run and org are named in the hover, and a status-bar eye shows the active file's percentage and toggles the painting. Coverage only ever comes from your own run or that explicit per-class load; it dims once you edit the file and is dropped when you switch orgs.
...
| `sfTestRunner.runWithLogs` | `false` | Keep each test method's debug log (the "with logs" chip mirrors this); ensures a USER_DEBUG trace flag for your user before the run. |
...
| `sfTestRunner.autoShowOutput` | `false` | Reveal the SF Tests output channel when a run starts or a recent run is loaded. The Results view already shows the run live. |
