...
- **Tests** — the target org as a dropdown with a DEV/SBX/PROD/SCR badge (production gets a warning tint), ⟳ to refresh the org list and ＋ to log in to another org. All/Selected tabs, a search box and a source filter sit above a tree of test classes with tri-state checkboxes down to the method. **Tests for active file** ticks the tests for the class open in the editor. Buttons: **Run Selected**, **Run All Local** (`--test-level RunLocalTests`, every test in the org except managed-package ones), and a ⋯ menu with **Run all tests in org (incl. managed)** and **Load recent run**. A "with coverage" chip decides whether the run asks for `--code-coverage`. While a run is on you see done/total, the elapsed time and a **Cancel** button.
